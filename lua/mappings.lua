-- Helpers
local function get_dir(filepath) return filepath:match '(.*/)' end

local function cmd_async(str) vim.api.nvim_input(str) end
...
if 'Change color on mode' and false then
  ...
  vim.api.nvim_create_autocmd({ 'InsertEnter' }, {
    callback = function() vim.api.nvim_set_hl(0, 'Normal', { bg = '#110000' }) end,
  })
...
-- end)
map('i', '<C-q>', function() vim.lsp.buf.hover() end)
